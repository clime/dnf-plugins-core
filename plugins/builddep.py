...
class BuildDepCommand(dnf.cli.Command):
    ...
    def _spec_deps(self, spec_fn):
        # pass spec through preprocessing if preproc-rpmspec is installed
        # shutil.which is accessible only since python 3.3
        temp = None
        if sys.version_info >= (3, 3) and shutil.which('preproc-rpmspec'):
